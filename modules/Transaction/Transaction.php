...
class Transaction extends Vtiger_CRMEntity {
	var $table_name = 'vtiger_transaction';
	var $table_index= 'transactionid';

	/**
	 * Mandatory table for supporting custom fields.
	 */
	var $customFieldTable = Array('vtiger_transactioncf', 'transactionid');

	/**
	 * Mandatory for Saving, Include tables related to this module.
	 */
	var $tab_name = Array('vtiger_crmentity', 'vtiger_transaction', 'vtiger_transactioncf');
	...
	var $related_tables = Array( 
					'vtiger_transactioncf' => Array('transactionid')
					);
	...
	var $tab_name_index = Array(
		...
		'vtiger_transaction'   => 'transactionid',
	    'vtiger_transactioncf' => 'transactionid');
	...
	var $list_fields = Array (
		...
		'Transaction No' => Array('transaction', 'transactionno'),
/*'Transaction No'=> Array('transaction', 'transactionno'),*/
...
/*'Transaction No'=> 'transactionno',*/
	...
	var $list_link_field = 'transactionno';
...
/*'Transaction No'=> 'transactionno',*/
	...
	var $popup_fields = Array ('transactionno');

	// For Alphabetical search
	var $def_basicsearch_col = 'transactionno';

	// Column value to use on detail view record text display
	var $def_detailview_recname = 'transactionno';

	// Used when enabling/disabling the mandatory fields for the module.
	// Refers to vtiger_field.fieldname values.
	var $mandatory_fields = Array('transactionno','assigned_user_id');

	var $default_order_by = 'transactionno';
	...
	*/
	function vtlib_handler($moduleName, $eventType) {
		global $adb;
 		if($eventType == 'module.postinstall') {
			...
			Transaction::checkWebServiceEntry();
			Transaction::createUserFieldTable($moduleName);
			Transaction::addInTabMenu($moduleName);
	...
	static function checkWebServiceEntry() {
		...
		$sql       =  "SELECT count(id) AS cnt FROM vtiger_ws_entity WHERE name = 'Transaction'";
		...
		if($adb->num_rows($result) > 0)
		{
			...
			if($no == 0)
			{
				...
				$ws_entitySql = "INSERT INTO vtiger_ws_entity ( id, name, handler_path, handler_class, ismodule ) VALUES".
						  " (?, 'Transaction','include/Webservices/VtigerModuleOperation.php', 'VtigerModuleOperation' , 1)";
